                                        ...
                                        <ul class="list-unstyled">
                                            <?php 
                                                foreach ($manual as $key => $value) {
                                                    ?>
                                                    <li class="<?php if(!empty($_GET['id'])){if($_GET['id']==$value->_id){echo 'mm-selected';}}?>"><a href="<?=Yii::$app->urlManager->createAbsoluteUrl('huong-dan-su-dung/'. $value->slug .'-' . (string)$value->_id)?>"><?=$value->icon.$value->name?></a>
                                                    <?php
                                                    ...
                                                    if(!empty($cate2) | !empty($post2)){
                                                        ...
                                                        ?>
                                                        <li class="<?php if(!empty($_GET['id'])){if($_GET['id']==$val->_id){echo 'mm-selected';}}?>"><a href="<?=Yii::$app->urlManager->createAbsoluteUrl('huong-dan-su-dung/'. $val->slug .'-' . (string)$val->_id)?>"><?=$val->name?></a>
                                                        <?php
                                                            ...
                                                            if(!empty($cate3) | !empty($post3)){
                                                                ...
                                                                ?>
                                                                <li class="<?php if(!empty($_GET['id'])){if($_GET['id']==$cat->_id){echo 'mm-selected';}}?>"><a href="<?=Yii::$app->urlManager->createAbsoluteUrl('huong-dan-su-dung/'. $cat->slug .'-' . (string)$cat->_id)?>"><?=$cat->name?></a>
                                                                <?php
                                                                }
                                                                foreach ($post3 as $key => $cat) {
                                                                ?>
                                                                    <li class="<?php if(!empty($_GET['id'])){if($_GET['id']==$cat->_id){echo 'mm-selected';}}?>"><a href="<?=Yii::$app->urlManager->createAbsoluteUrl('huong-dan-su-dung/' . (string)$cat->_id)?>"><?=$cat->name?></a>
                                                                <?php }
                                                                echo '</ul>';
                                                            }
                                                            echo '</li>';
                                                        ...
                                                        ?>
                                                            <li class="<?php if(!empty($_GET['id'])){if($_GET['id']==$val->_id){echo 'mm-selected';}}?>"><a href="<?=Yii::$app->urlManager->createAbsoluteUrl('huong-dan-su-dung/' . (string)$val->_id)?>"><?=$val->name?></a>
                                                        <?php           
                                                                }
                                                        echo '</ul>';
                                                    }
                                ...
                                <div class="col-md-9 col-sm-7 col-xs-12 box-content-item">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 col-sm-12 col-xs-12">
                                                ...
                                                <h5><a class="read-more" href="<?=Yii::$app->urlManager->createAbsoluteUrl('huong-dan-su-dung/'. $value->slug .'-' . (string)$value->_id)?>">Xem thêm &raquo;</a></h5>    
                                        </div>
                                        <?php }?>
                                    </div>
                                </div>
