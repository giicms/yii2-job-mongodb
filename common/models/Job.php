...
class Job extends ActiveRecord
{
    ...
    {
        $arr = explode('-', $this->budget_type);
        return number_format($arr[0], 0, '', '.').' - '.number_format($arr[1], 0, '', '.').' VNĐ';
    }
    ...
    {
        $packet = BudgetPacket::find()->where(['sector_id' => $this->sector_id])->one();
        var_dump($this->sector_id); exit;
        $models = BudgetPrice::find()->where(['bg_id' => $packet->id])->all();
        if ($models)
        {
            foreach ($models as $value)
            {
