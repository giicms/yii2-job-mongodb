...
class Imagicks
{
    ...
    public static function convertImage($path, $name)
    {
        $nameArray = explode('.', $name);

        $image = new \Imagick($path . $name);
        $image->setImageFormat('png');
        $image->writeImage($path . $nameArray[0] . '.png');

        header('Content-Type: image/jpeg');
        $url = '/uploads/' . $nameArray[0] . '.png';

        return $url;
    }

    public static function cropImage($path, $name, $startX, $startY, $width, $height)
    {
        $nameArray = explode('.', $name);
        $imagick = new \Imagick(realpath($path . $name));
        $imagick->cropImage($width, $height, $startX, $startY);
        $imagick->writeImage($path . $width . '-' . $nameArray[0] . '.png');
        return $width . '-' . $nameArray[0] . '.png';
