...
class MongodbManager extends BaseManager {
    ...
    public function getTask($name) {
        ...
        return $item instanceof \yii\rbac\Item && $item->type == Item::TYPE_TASK ? $item : null;
    }
...
//            ->remove(['parent' => $parent->name]) === true;
    ...
    public function getChildren($name) {
        ...
        $query = (new Query)
                ->select(['name','parent' ,'type', 'description', 'rule_name', 'data', 'created_at', 'updated_at'])
                ->from($this->itemTable)
                ->where(['name' => $names])->orderBy(['description'=>SORT_ASC]);
