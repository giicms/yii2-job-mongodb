                        ...
                        <ul class="messages-bar">
                            ...
                            if (!empty($model)) {
                                foreach ($model as $value) {
                                    ?>
                                    <li>
                                        <a href="<?= Yii::$app->urlManager->createAbsoluteUrl('tin-nhan/' . $value->owners->slug . '?id=' . (string) $value->_id) ?>"  class="<?= $value->status == 2 ? "active" : "" ?>">
                                            <img class="avatar-32" src="<?= Yii::$app->params['url_file'] ?>/thumbnails/<?= !empty($value->owners->avatar) ? '60-' . $value->owners->avatar : "avatar.png" ?>">
                                            <div class="not-inf">
