...
class CustomerController extends BackendController {
    ...
    public function actionIndex() {
        $this->canUser();
        $cate = BlogCategory::find()->where(['alias' => 'rev'])->orderBy(['order' => SORT_ASC])->one();
        $dataProvider = new ActiveDataProvider([
            'query' => Posts::find()->where(['alias'=>'rev', 'category_id'=>(string)$cate->_id])->orderBy(['updated_at' => SORT_DESC])
        ]);
        ...
        if ($new->load(Yii::$app->request->post())) {
            ...
            if ($new->save()) {
                $this->render('index', [ 'dataProvider' => $dataProvider, 'model'=>$model, 'new'=>$new, 'id'=>(string)$cate->_id]);
            }
        }
        return $this->render('index', [
                    'dataProvider' => $dataProvider, 'model'=>$model, 'new'=>$new, 'id'=>(string)$cate->_id
        ]);
    }
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()){
                $this->redirect('/customer/index/'.$model->category_id);
            }  
        } else {
    ...
    public function actionDelete($id) {
        ...
        return $this->redirect('/customer/index/'.$model->category_id);
    }
